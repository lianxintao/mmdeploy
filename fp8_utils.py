...
from sglang.srt.layers.quantization.fp8_kernel import (
    ...
    mxfp8_block_scaled_matmul_triton_sm89,
...
# Ada (SM89): has FP8 tensor cores but no TMA, so it uses the non-TMA MXFP8 path.
_is_sm89 = _is_cuda and get_device_capability() == (8, 9)
...
def _raw_triton_mxfp8_blockscaled_linear(
    ...
    if not (_is_cuda and (_is_sm100_supported or _is_sm120_supported or _is_sm89)):
        raise RuntimeError(
            "MXFP8 dense linear requires SM89 (Ada) or Blackwell GPUs (SM100/SM120)."
        )
    ...
    if _is_sm89:
        # Ada has no TMA; use the non-TMA FP8-tensor-core kernel with the plain
        # (M, K//32) UE8M0 scale layout. The kernel masks partial M/N tiles, so
        # no padding/swizzle is needed (cheaper on the decode latency path).
        output = mxfp8_block_scaled_matmul_triton_sm89(
            q_input,
            x_scale_u8,
            weight.contiguous(),
            weight_scale,
    ...
    # ---- Blackwell (SM100/SM120) TMA path ----
