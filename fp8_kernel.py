...
def _get_mxfp8_sm89_configs():
    # Decode-focused (small M): small BLOCK_M, narrow-to-wide BLOCK_N for
    # occupancy. Curated (not a full sweep) to keep first-call autotune compile
    # time bounded. BLOCK_K is derived from GROUPS_PER_LOAD outside the config.
    return [
        triton.Config(
            {"BLOCK_M": bm, "BLOCK_N": bn, "GROUP_SIZE_M": gm},
            num_warps=nw,
            num_stages=ns,
        )
        for (bm, bn, gm, nw, ns) in (
            (16, 64, 1, 4, 3),
            (16, 128, 1, 4, 3),
            (16, 256, 1, 8, 3),
            (32, 64, 1, 4, 4),
...
@triton.jit
def _mxfp8_split4(x):
    """Split a (R, 32, 4) tile into four (R, 32) group tiles.

    Returns the groups in extraction order [0, 2, 1, 3] (the natural order of
    nested ``tl.split`` over a (R, 32, 2, 2) reshape). Callers must pair each
    returned tile with the scale of its matching group id, which is why the
    helper is used symmetrically for A, B and their per-group scales.
    """
    lo, hi = tl.split(tl.reshape(x, (x.shape[0], 32, 2, 2)))  # each (R, 32, 2)
    g0, g2 = tl.split(lo)
    g1, g3 = tl.split(hi)
    return g0, g2, g1, g3


@triton.autotune(configs=_get_mxfp8_sm89_configs(), key=["N", "K"])
@triton.jit
def _mxfp8_block_scaled_matmul_kernel_sm89(
    A,
    B,
    C,
    As,
    ...
    BLOCK_N: tl.constexpr,
    GROUP_SIZE_M: tl.constexpr,
    GROUPS_PER_LOAD: tl.constexpr,  # MX groups (of 32) loaded per K iteration
):
    """Non-TMA MXFP8 block-scaled matmul for GPUs without TMA (e.g. SM89/Ada).

    Computes ``C = (A * a_scale) @ (B * b_scale).T`` where A is (M, K) and B is
    (N, K) FP8 e4m3, with per-32-element UE8M0 (uint8) scales ``As`` (M, K//32)
    and ``Bs`` (N, K//32). The FP8 tensor cores are used directly with the
    output-scaling trick ``acc += dot(a_g, b_g.T) * a_s[:, None] * b_s[None, :]``
    applied per 32-element MX group.

    To stay memory-efficient on the decode (small-M, weight-bound) path, each K
    iteration issues one wide contiguous load of ``GROUPS_PER_LOAD * 32``
    elements per row, then splits it into 32-wide groups for the scaled
    sub-dots. ``GROUPS_PER_LOAD == 4`` (128-wide loads) is the fast path;
    ``GROUPS_PER_LOAD == 1`` is the general fallback for K not divisible by 128.
    """
    BLOCK_K: tl.constexpr = GROUPS_PER_LOAD * 32
    pid = tl.program_id(axis=0)
    num_pid_m = tl.cdiv(M, BLOCK_M)
    ...
    for _ in range(0, tl.cdiv(K, BLOCK_K)):
        a = tl.load(a_ptrs)  # (BLOCK_M, BLOCK_K) wide contiguous load
        b = tl.load(b_ptrs)  # (BLOCK_N, BLOCK_K)

        if GROUPS_PER_LOAD == 1:
            # Decode UE8M0 (uint8 exponent e) -> fp32 multiplier 2^(e-127) by
            # placing e into the fp32 exponent field (zero mantissa). e=0 -> 0.0,
            # padding e=127 -> 1.0.
            a_s = ((tl.load(as_ptrs).to(tl.uint32)) << 23).to(tl.float32, bitcast=True)
            b_s = ((tl.load(bs_ptrs).to(tl.uint32)) << 23).to(tl.float32, bitcast=True)
            accumulator += tl.dot(a, tl.trans(b)) * a_s[:, None] * b_s[None, :]
        else:  # GROUPS_PER_LOAD == 4
            a4 = tl.trans(tl.reshape(a, (BLOCK_M, 4, 32)), 0, 2, 1)  # (BM, 32, 4)
            b4 = tl.trans(tl.reshape(b, (BLOCK_N, 4, 32)), 0, 2, 1)  # (BN, 32, 4)
            a0, a2, a1, a3 = _mxfp8_split4(a4)
            b0, b2, b1, b3 = _mxfp8_split4(b4)
            as0 = ((tl.load(as_ptrs + 0 * stride_as_k).to(tl.uint32)) << 23).to(
                tl.float32, bitcast=True
            )
            as1 = ((tl.load(as_ptrs + 1 * stride_as_k).to(tl.uint32)) << 23).to(
                tl.float32, bitcast=True
            )
            as2 = ((tl.load(as_ptrs + 2 * stride_as_k).to(tl.uint32)) << 23).to(
                tl.float32, bitcast=True
            )
            as3 = ((tl.load(as_ptrs + 3 * stride_as_k).to(tl.uint32)) << 23).to(
                tl.float32, bitcast=True
            )
            bs0 = ((tl.load(bs_ptrs + 0 * stride_bs_k).to(tl.uint32)) << 23).to(
                tl.float32, bitcast=True
            )
            bs1 = ((tl.load(bs_ptrs + 1 * stride_bs_k).to(tl.uint32)) << 23).to(
                tl.float32, bitcast=True
            )
            bs2 = ((tl.load(bs_ptrs + 2 * stride_bs_k).to(tl.uint32)) << 23).to(
                tl.float32, bitcast=True
            )
            bs3 = ((tl.load(bs_ptrs + 3 * stride_bs_k).to(tl.uint32)) << 23).to(
                tl.float32, bitcast=True
            )
            accumulator += tl.dot(a0, tl.trans(b0)) * as0[:, None] * bs0[None, :]
            accumulator += tl.dot(a1, tl.trans(b1)) * as1[:, None] * bs1[None, :]
            accumulator += tl.dot(a2, tl.trans(b2)) * as2[:, None] * bs2[None, :]
            accumulator += tl.dot(a3, tl.trans(b3)) * as3[:, None] * bs3[None, :]

        a_ptrs += BLOCK_K * stride_ak
        b_ptrs += BLOCK_K * stride_bk
        as_ptrs += GROUPS_PER_LOAD * stride_as_k
        bs_ptrs += GROUPS_PER_LOAD * stride_bs_k
...
def mxfp8_block_scaled_matmul_triton_sm89(
    a: torch.Tensor,
    a_scale: torch.Tensor,
    b: torch.Tensor,
    b_scale: torch.Tensor,
    ...
    """Non-TMA MXFP8 block-scaled matmul launcher (targets SM89/Ada).

    Args:
        a: (M, K) FP8 e4m3.
        a_scale: (M, K // 32) UE8M0 uint8 scales (plain layout, no TMA swizzle).
        b: (N, K) FP8 e4m3.
        b_scale: (N, K // 32) UE8M0 uint8 scales.
        output_dtype: float16, bfloat16 or float32.

    Returns:
        (M, N) tensor of ``output_dtype`` holding ``(a*a_scale) @ (b*b_scale).T``.
    """
    assert a.dtype == torch.float8_e4m3fn, f"a must be FP8 e4m3, got {a.dtype}"
    assert b.dtype == torch.float8_e4m3fn, f"b must be FP8 e4m3, got {b.dtype}"
    assert a_scale.dtype == torch.uint8, f"a_scale must be uint8, got {a_scale.dtype}"
    assert b_scale.dtype == torch.uint8, f"b_scale must be uint8, got {b_scale.dtype}"
    M, K = a.shape
    N, K_b = b.shape
    assert K == K_b, f"{K=} does not match {K_b=}"
    assert K % 32 == 0, f"{K=} must be divisible by 32 (MX group size)"
    assert a_scale.shape == (M, K // 32), f"{a_scale.shape=} != {(M, K // 32)}"
    assert b_scale.shape == (N, K // 32), f"{b_scale.shape=} != {(N, K // 32)}"

    # 128-wide loads (4 MX groups) are the fast path; fall back to 32-wide loads
    # when K is not a multiple of 128.
    groups_per_load = 4 if K % 128 == 0 else 1
    output = torch.empty((M, N), dtype=output_dtype, device=a.device)

    def grid(META):
        return (triton.cdiv(M, META["BLOCK_M"]) * triton.cdiv(N, META["BLOCK_N"]),)

    _mxfp8_block_scaled_matmul_kernel_sm89[grid](
        a,
        b,
        output,
        a_scale,
        ...
        b_scale.stride(0),
        b_scale.stride(1),
        GROUPS_PER_LOAD=groups_per_load,
    )
    return output
