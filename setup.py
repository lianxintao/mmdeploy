...
# Qwen3-MoE keeps RadixAttention under ``self_attn.attn``.  Quantization
# exporters store calibrated KV-cache scales at several HF-side locations:
# compressed-tensors/llm-compressor uses ``self_attn.{k,v}_scale`` while older
# compressed-tensors and ModelOpt checkpoints may attach them to k_proj/v_proj.
# Remap all supported spellings before the stacked qkv loader can consume or
# silently skip them.
QWEN3_MOE_KV_SCALE_MAPPER = WeightsMapper(
    orig_to_new_substr={
        ".self_attn.k_proj.output_scale": ".self_attn.attn.k_scale",
        ".self_attn.v_proj.output_scale": ".self_attn.attn.v_scale",
...
class Qwen3MoeForCausalLM(nn.Module):
    ...
    def load_weights(
        ...
        weights = QWEN3_MOE_KV_SCALE_MAPPER.apply(weights)
